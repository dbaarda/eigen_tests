...
set(eigen_tests_WEB_DIR "${CMAKE_SOURCE_DIR}/web")
set(eigen_tests_COI_WORKER "${eigen_tests_WEB_DIR}/coi-serviceworker.min.js")
set(eigen_tests_SHELL_TEMPLATE "${eigen_tests_WEB_DIR}/shell_template.html.in")
set(eigen_tests_INDEX_TEMPLATE "${eigen_tests_WEB_DIR}/index_template.html.in")
...
if(EMSCRIPTEN)
  ...
  # Set WASM build flag
  set(eigen_tests_WASM_BUILD ON CACHE BOOL "Building for WebAssembly" FORCE)

  # Sanitizers don't work with Emscripten
  foreach(sanitizer ADDRESS LEAK UNDEFINED THREAD MEMORY)
    set(eigen_tests_ENABLE_SANITIZER_${sanitizer} OFF CACHE BOOL "Not supported with Emscripten")
  endforeach()

  # Disable static analysis and strict warnings for Emscripten builds
  foreach(option CLANG_TIDY CPPCHECK WARNINGS_AS_ERRORS)
    set(eigen_tests_ENABLE_${option} OFF CACHE BOOL "Disabled for Emscripten")
  endforeach()
  ...
  set(eigen_tests_WASM_INITIAL_MEMORY "33554432" CACHE STRING
      "Initial WASM memory in bytes (default: 32MB)")
  set(eigen_tests_WASM_PTHREAD_POOL_SIZE "4" CACHE STRING
      "Pthread pool size for WASM builds (default: 4)")
  set(eigen_tests_WASM_ASYNCIFY_STACK_SIZE "65536" CACHE STRING
...
function(eigen_tests_configure_wasm_target target)
  if(EMSCRIPTEN)
    ...
    set_property(GLOBAL APPEND PROPERTY eigen_tests_WASM_TARGETS "${target}")
    set_property(GLOBAL PROPERTY eigen_tests_WASM_TARGET_${target}_TITLE "${WASM_TITLE}")
    set_property(GLOBAL PROPERTY eigen_tests_WASM_TARGET_${target}_DESCRIPTION "${WASM_DESCRIPTION}")
    set_property(GLOBAL PROPERTY eigen_tests_WASM_TARGET_${target}_OUTPUT_NAME "${OUTPUT_NAME}")

    target_compile_definitions(${target} PRIVATE eigen_tests_WASM_BUILD=1)
    ...
    target_link_options(${target} PRIVATE
      ...
      "-sUSE_PTHREADS=1"
      "-sPROXY_TO_PTHREAD=1"
      "-sPTHREAD_POOL_SIZE=${eigen_tests_WASM_PTHREAD_POOL_SIZE}"
      # Enable asyncify for emscripten_sleep and async operations
      "-sASYNCIFY=1"
      "-sASYNCIFY_STACK_SIZE=${eigen_tests_WASM_ASYNCIFY_STACK_SIZE}"
      # Memory configuration
      "-sALLOW_MEMORY_GROWTH=1"
      "-sINITIAL_MEMORY=${eigen_tests_WASM_INITIAL_MEMORY}"
    ...
    if(EXISTS "${eigen_tests_SHELL_TEMPLATE}")
      configure_file(
        "${eigen_tests_SHELL_TEMPLATE}"
        "${CONFIGURED_SHELL}"
        @ONLY
      )
      ...
      message(FATAL_ERROR "Shell template not found: ${eigen_tests_SHELL_TEMPLATE}")
    endif()

    # Copy service worker to target build directory for standalone target builds
    if(EXISTS "${eigen_tests_COI_WORKER}")
      add_custom_command(TARGET ${target} POST_BUILD
        COMMAND ${CMAKE_COMMAND} -E copy_if_different
          "${eigen_tests_COI_WORKER}"
...
function(eigen_tests_create_web_dist)
  if(NOT EMSCRIPTEN)
    return()
  endif()
  ...
  get_property(WASM_TARGETS GLOBAL PROPERTY eigen_tests_WASM_TARGETS)
  ...
  foreach(target ${WASM_TARGETS})
    get_property(TITLE GLOBAL PROPERTY eigen_tests_WASM_TARGET_${target}_TITLE)
    get_property(DESCRIPTION GLOBAL PROPERTY eigen_tests_WASM_TARGET_${target}_DESCRIPTION)
...
")
  ...
  if(EXISTS "${eigen_tests_INDEX_TEMPLATE}")
    configure_file("${eigen_tests_INDEX_TEMPLATE}" "${INDEX_OUTPUT}" @ONLY)
  else()
    message(WARNING "Index template not found: ${eigen_tests_INDEX_TEMPLATE}")
  endif()
  ...
  foreach(target ${WASM_TARGETS})
    ...
    get_property(OUTPUT_NAME GLOBAL PROPERTY eigen_tests_WASM_TARGET_${target}_OUTPUT_NAME)
